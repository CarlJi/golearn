...
func (b *builder) runTest(a *action) error {
	...
	log.Infof("此处开始真正执行测试用例，从用编译好的二进制来运行，参数: %v \n", args)
...
func cp(src string) {
	dest := "/Users/jicarl/qbox/golearn/src/carlji.com/experiments/gocover"
	log.Infof("复制该测试文件到: %s \n", dest)

	cmd := exec.Command("sh", "-c", fmt.Sprintf("cp %s %s", src, dest))
